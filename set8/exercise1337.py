...
def string_list_please() -> list:
    ...
    list_str = ["dog", "cat", "help", "me", "please"]
    return list_str
...
def set_it_on_fire(input_string="very naughty boy") -> str:
    ...
    """
    flame = list(input_string)
    flame = input_string.upper()
    flame = "🔥".join(flame)
    return "🔥" + flame + "🔥"
...
def fast_filler(number_of_words=200) -> str:
    ...
    for _ in range(number_of_words):
        ...
        word_index = random.randint(0, 3)
        try:
            list_of_words.append(my_dict[word_length][word_index])
        except KeyError:
            list_of_words.append(my_dict[str(word_length)][word_index])
